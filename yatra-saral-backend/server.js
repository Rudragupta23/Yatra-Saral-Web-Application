...
    db = client.db('yatra-saral');
    ...
    // **CRITICAL FIX: START SERVER ONLY AFTER DB CONNECTION IS SUCCESSFUL**
    app.listen(PORT, () => {
        console.log(`✅ Server running on http://localhost:${PORT}`);
    });
  })
  .catch(err => {
    console.error('❌ Failed to connect to MongoDB. Exiting application.', err);
    // CRITICAL FIX: Shut down the application process if DB connection fails
    process.exit(1); 
  });
...
// The app.listen() block has been moved up inside the database connection promise.
// const PORT = 5000;
// app.listen(PORT, () => {
//   console.log(`✅ Server running on http://localhost:${PORT}`);
// });
